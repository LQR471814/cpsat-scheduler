...
from cpsatscheduler.backend import (
    ConfigBuilder,
    Model,
    ScheduledTask,
    Task,
...
from cpsatscheduler.backend.config import Solution
from cpsatscheduler.frontend import cost_topo
from cpsatscheduler.frontend.units import hour_4, timescale_names
...
class Schedule:
    ...
    def task(
        ...
        if start_after is not None and start_after > self.horizon[1]:
            raise ValueError("start_after cannot be after horizon upper bound")
    ...
    def event(self, name: str, start: datetime, end: datetime, unit=hour_4):
        ...
        if start > self.horizon[1]:
            raise ValueError("start cannot be after horizon upper bound")
        ...
        while current_inst < task_end:
            ...
            start_in_inst = current_inst < task_start < next_inst
            end_in_inst = current_inst < task_end < next_inst
