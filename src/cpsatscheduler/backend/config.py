...
from cpsatscheduler.backend.print import print_vars
from cpsatscheduler.backend.units import atomic_unit, task_unit
...
class ModelProps:
    ...
    def __get_max_scaling_factor(self, config: Config) -> atomic_unit:
        ...
        for u in sorted_timescales[1:]:
            scale = u // prev
            max_scaling_factor = max(max_scaling_factor, scale)
    ...
    def __compute_real_end_bounds(
        ...
        for cfg in t.cost_configs:
            if len(cfg.children) == 0:
                ...
                min_end = min(min_end, end_lb)

                end_ub = start_ub + t.timescale_unit
                max_end = max(max_end, end_ub)
    ...
    def __compute_cost_bounds(self, task_id: int) -> tuple[int, int]:
        ...
        for cfg in t.cost_configs:
            for intv in cfg.costs:
                min_cost = min(min_cost, intv.cost)
                max_cost = max(max_cost, intv.cost)
    ...
    def __compute_dur_bounds(self, task_id: int) -> tuple[atomic_unit, atomic_unit]:
        ...
        for cfg in t.cost_configs:
            if cfg.duration is not None:
                max_dur = max(max_dur, cfg.duration)
                min_dur = min(min_dur, cfg.duration)
                continue
            ...
            min_dur = min(min_dur, min_dur_sum)
            max_dur = max(max_dur, max_dur_sum)
...
def assert_non_overflow(config: Config, scheduled: list[ScheduledTask]):
    ...
    for unit, unit_buckets in unit_bucket_durations.items():
        for filled in unit_buckets.values():
            assert filled <= unit
