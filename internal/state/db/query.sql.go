...
const createChildrenConfig = `-- name: CreateChildrenConfig :one
...
func (q *Queries) CreateChildrenConfig(ctx context.Context, arg CreateChildrenConfigParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, createChildrenConfig,
		arg.Task,
		arg.Desc,
		arg.Deadline,
		arg.ExpCost,
...
func (q *Queries) DeleteSchedule(ctx context.Context, profile int64) error {
	_, err := q.db.ExecContext(ctx, deleteSchedule, profile)
	return err
}

const getDurConfig = `-- name: GetDurConfig :one
select id, task, pes, exp, opt, deadline, total_cost from dur_config where task = ?
`
...
select id, name, atomic_timescale_duration, universe_start, pert_gen_choices from profile where id = ?
`
...
func (q *Queries) SaveScheduledTask(ctx context.Context, arg SaveScheduledTaskParams) error {
	_, err := q.db.ExecContext(ctx, saveScheduledTask,
		arg.Task,
		arg.Profile,
		arg.Start,
		arg.End,
