...
def req [method: string]: any -> any {
    buf curl -d (to json) --unix-socket $SOCKET_PATH --protocol grpc --http2-prior-knowledge --schema ../proto $"http://localhost/($API_SERVICE_PATH)/($method)" | from json # nu-lint-ignore: catch_builtin_error_try
}
