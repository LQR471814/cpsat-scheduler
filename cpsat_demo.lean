...
def demoModel? (tableA : CostTable atomic) (tableB : CostTable unit4)
  ...
  let result := Builder.run do
    ...
    constrainCostByTable a.vars tableA
    constrainCostByTable b.vars tableB
    constrainCostByTable c.vars tableC
    let _ ← minimizeCostSum #[a.vars, b.vars, c.vars]
    pure ()
  result.1.finalize?
...
def main : IO Unit := do
  ...
  let (⟨hA, hB, hC⟩, results) ← PertM.run runtime do
    let hA ← requestCostTable configA
    let hB ← requestCostTable configB
    let hC ← requestCostTable configC
    pure (hA, hB, hC)
