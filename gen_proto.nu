let python_out = (pwd) | path join solver/src # nu-lint-ignore: dont_mix_different_effects
let proto: string = (pwd) | path join proto

print "gen python..."

let solverpb: list<string> = ls (
  $proto
  | path join solverpb/**/*.proto
  | into glob
) | get name

let commonpb: list<string> = ls (
  $proto
  | path join commonpb/**/*.proto
  | into glob
) | get name

python-grpc-tools-protoc -I $proto --python_out $python_out --pyi_out $python_out --grpc_python_out $python_out ...$solverpb ...$commonpb | complete | print

print "gen buf..."

buf generate | complete | print

# move nushell gen files into the right directory
rm --recursive ./cli/lib/proto/commonpb
rm --recursive ./cli/lib/proto/solverpb
